...
@section('content')
    ...
    <div class="panel panel-flat">
        <div class="panel-heading">
            <h5 class="panel-title">      {{__('slider.index')}}            </h5>
        ...
        <a href="{{route('dashboard.slider.create')}}">
                <button type="button" class="btn btn-primary btn-ladda btn-ladda-progress ladda-button pull-right legitRipple" data-style="zoom-in">
                    <span class="ladda-label">  {{__('slider.add')}}    </span>
        ...
        <table class="table datatable-button-init-basic" id="first">
            ...
            <tr>
                ...
                <th> {{__('slider.description')}}     </th>

                
                <th> {{__('status')}}     </th>

                <th> {{__('slider.proces')}}     </th>
            </tr>
            </thead>
            <tbody>
            @foreach($items as $key=>$item)
                <tr>
                    ...
                    <td><img src=" {{getimg($item->image_ar)}}" style="width: 100px; height: 100px; background-color:yellowgreen" class="img-circle"></td>
                    <td>{{$item->lg_text_ar}}</td>

                    <td>{{ statusstring($item->status)}}</td>

                    {!!Form::open( ['route' => ['dashboard.slider.destroy',$item->id] ,'id'=>'delete-form'.$item->id, 'method' => 'Delete']) !!}
                    {!!Form::close() !!}
                    ...
                    <td>
                        ...
                        <a href="{{route('dashboard.slider.edit',[$item])}}" data-toggle="tooltip" data-original-title="update"> <i class="icon-pencil7 text-inverse" style="margin-left: 10px"></i> </a>
                        @endcan
