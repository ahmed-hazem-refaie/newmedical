...
@section('content')
    ...
    <div class="panel panel-flat">
        <div class="panel-heading">
            <h5 class="panel-title">      {{__('service.index')}}            </h5>
        ...
        <a href="{{route('dashboard.service.create')}}">
                <button type="button" class="btn btn-primary btn-ladda btn-ladda-progress ladda-button pull-right legitRipple" data-style="zoom-in">
                    <span class="ladda-label">  {{__('service.add')}}    </span>
        ...
        <table class="table datatable-button-init-basic" id="first">
            ...
            <tr>
                ...
                <th> {{__('service.logo')}}     </th>
                <th> {{__('service.image')}}     </th>
                <th> {{__('status')}}     </th>

                <th> {{__('service.proces')}}     </th>
            </tr>
            </thead>
            <tbody>
            @foreach($items as $key=>$item)
                <tr>
                    ...
                    <td>{{$item->name_en}}</td>
                    <td>{{$item->title_en}}</td>
                    <td><img src=" {{($item->logo)}}" style="width: 100px; height: 100px" class="img-circle"></td>
                    <td><img src=" {{getimg($item->image_en)}}" style="width: 100px; height: 100px" class="img-circle"></td>
                     {!!Form::open( ['route' => ['dashboard.service.destroy',$item->id],'id'=>'delete-form'.$item->id, 'method' => 'Delete']) !!}
                    {!!Form::close() !!}
                    ...
                    <td>
                        ...
                        <a href="{{route('dashboard.service.edit',[$item])}}" data-toggle="tooltip" data-original-title="update"> <i class="icon-pencil7 text-inverse" style="margin-left: 10px"></i> </a>
                        @endcan
