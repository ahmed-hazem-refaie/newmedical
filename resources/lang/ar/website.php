<?php

return [
    'search' => 'بحث',
    'categories' => 'الأقسام',
    'recent post' => 'المقالات الحديثة',
    'related post' => 'مقالات مشابهه',
    'comments' => 'التعليقات',
    'leave a reply' => 'اترك تعليق',
    'Your email address will not be published. Required fields are marked *' => 'لن يتم نشر عنوان بريدك الإلكتروني. الحقول المطلوبة محددة *',
    'first name' => 'الأسم الأول',
    'last name' => 'الأسم الأخير',
    'email' => 'البريد الألكترونى',
    'message' => 'نص التعليق',
    'post comment' => 'نشر التعليق',
    'home' => 'الرئيسية',
    'blog details' => 'تفاصيل المقال',
    'close' => 'اغلاق',
    'blogs' => 'المقالات',
    'reply' => 'رد',
    'not found blogs' => 'لا يوجد مقالات',
    'Your comment Added Successfully' => 'تمت إضافة تعليقك بنجاح',
    'location' => 'الموقع',
    'tel' => 'تليفون',
    'information' => 'معلومات',
    'copywright_2' => 'كل الحقوق محفوظة',
    'copywright' => 'الحقوق',
    'typeemail' => 'اكتب البريد الألكترونى',
    'subscribe' => 'اشتراك',
    'contacts' => 'اتصل بنا ',
    'services' => 'الخدمات',





    "email" => "البريد الإلكتروني",
    "location" => "الموقع",
    'tel' => 'تليفون',
    "information" => "معلومات",
    'copywright_2' => 'جميع الحقوق محفوظة',
    'copywright' => 'حقوق النشر',
    'typeemail' => 'اكتب البريد الإلكتروني',
    "اشتراك" => "اشتراك",
    "contacts" => "اتصل بنا",
    "service" => "الخدمة",
    "home" => "الصفحة الرئيسية",
    'blogs' => 'اخبار',
    'list' => 'عرض',
    'success contact'=>"لقد تلقينا رسالتك وسنتواصل معك قريبًا",
    'phone' => 'تليفون',
    "subject" => "الموضوع",
    'message' => 'رسالة',
    'sendmessage' => 'إرسال رسالة',
    "name" => "الاسم",
    'call' => 'اتصال',
    'contact' => ' اتصل بنا ',
    "web" => "الويب",
    'Services Link' => 'خدماتنا',
    'service details' => 'تفاصيل الخدمة',
    'servicedetails'=>'اعرف المزيد عن هذه الخدمة',
    'blogtitle' => 'أخبار خطة الاستثمار العالمية',
    'blogtitleinfo' => "مجموعه من الاخبار التي تذيدك معرفه بأسرار الادارات والنجاحات المستقبلية وتذيد وعيك لإدارة مشاريعك",





];
