...
@section('content')
    ...
    <div class="panel panel-flat">
        <div class="panel-heading">
            <h5 class="panel-title">      {{__('maincategory.index')}}            </h5>
        ...
        <div class="panel-body">
            ...
            {{__('maincategory.index')}}
        <a href="{{route('dashboard.maincategory.create')}}">
                <button type="button" class="btn btn-primary btn-ladda btn-ladda-progress ladda-button pull-right legitRipple" data-style="zoom-in">
                    <span class="ladda-label">  {{__('maincategory.add')}}    </span>
        ...
        <table class="table datatable-button-init-basic" id="first">
            ...
            <tr>
                <th> # </th>
                <th> {{__('maincategory.name')}}     </th>
                <th> {{__('maincategory.title')}}     </th>
                <th> {{__('maincategory.logo')}}     </th>
                <th> {{__('maincategory.image')}}     </th>
                <th> {{__('status')}}     </th>

                <th> {{__('maincategory.proces')}}     </th>
            </tr>
            </thead>
            <tbody>
            @foreach($items as $key=>$item)
                <tr>
                    ...
                    <td><img src=" {{getimg($item->image)}}" style="width: 100px; height: 100px" class="img-circle"></td>
                     {!!Form::open( ['route' => ['dashboard.maincategory.destroy',$item->id] ,'id'=>'delete-form'.$item->id, 'method' => 'Delete']) !!}
                    {!!Form::close() !!}
                    ...
                    <td>
                        ...
                        <a href="{{route('dashboard.maincategory.product.index',[$item])}}" data-toggle="tooltip" data-original-title="update"> <i class="fa fa-shopping-cart text-inverse" style="margin-left: 10px"></i> </a>
                        @endcan
                        @can('dashboardblogs_edit')
                            
                        <a href="{{route('dashboard.maincategory.edit',[$item])}}" data-toggle="tooltip" data-original-title="update"> <i class="icon-pencil7 text-inverse" style="margin-left: 10px"></i> </a>
                        @endcan
