...
@section('content')
    ...
    <div class="panel panel-flat">
        <div class="panel-heading">
            <h5 class="panel-title">      {{__('setting.index')}}            </h5>
        ...
        <a href="{{route('dashboard.setting.create')}}">
                <button type="button" class="btn btn-primary btn-ladda btn-ladda-progress ladda-button pull-right legitRipple" data-style="zoom-in">
                    <span class="ladda-label">  {{__('setting.add')}}    </span>
        ...
        <table class="table datatable-button-init-basic" id="first">
            ...
            <tr>
                <th> # </th>
                <th> {{__('setting.name_ar')}}     </th>
                <th> {{__('setting.name_en')}}     </th>
                <th> {{__('setting.icon')}}     </th>

                <th> {{__('setting.proces')}}     </th>
            </tr>
            </thead>
            <tbody>
            @foreach($items as $key=>$item)
                <tr>
                    ...
                    <td>{{$item->icon}}</td>
                     ...
                     {!!Form::open( ['route' => ['dashboard.setting.destroy',$item->id] ,'id'=>'delete-form'.$item->id, 'method' => 'Delete']) !!}
                    {!!Form::close() !!}
                    <td>
                        <a href="{{route('dashboard.setting.field.index',[$item])}}" data-toggle="tooltip" data-original-title="تعديل"> <i class="fa fa-comments text-inverse" style="margin-left: 10px"></i> </a>

                        <a href="{{route('dashboard.setting.edit',[$item])}}" data-toggle="tooltip" data-original-title="تعديل"> <i class="icon-pencil7 text-inverse" style="margin-left: 10px"></i> </a>
