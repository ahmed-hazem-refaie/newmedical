...
@section('content')
    ...
    <div class="panel panel-flat">
        <div class="panel-heading">
            <h5 class="panel-title">      {{__('workcategory.index')}}            </h5>
        ...
        <div class="panel-body">
            ...
            {{__('workcategory.index')}}
        <a href="{{route('dashboard.workcategory.create')}}">
                <button type="button" class="btn btn-primary btn-ladda btn-ladda-progress ladda-button pull-right legitRipple" data-style="zoom-in">
                    <span class="ladda-label">  {{__('workcategory.add')}}    </span>
        ...
        <table class="table datatable-button-init-basic" id="first">
            ...
            <tr>
                <th> # </th>
                <th> {{__('workcategory.name')}}     </th>

                <th> {{__('workcategory.description')}}     </th>


                
                <th> {{__('workcategory.proces')}}     </th>
            </tr>
            </thead>
            <tbody>
            @foreach($items as $key=>$item)
                <tr>
                    ...
                    {!!Form::open( ['route' => ['dashboard.workcategory.destroy',$item->id] ,'id'=>'delete-form'.$item->id, 'method' => 'Delete']) !!}
                    {!!Form::close() !!}
                    ...
                    <td>
                        ...
                        @can('dashboardworkcategorys_edit')
                            
                        <a href="{{route('dashboard.workcategory.edit',[$item])}}" data-toggle="tooltip" data-original-title="تعديل"> <i class="icon-pencil7 text-inverse" style="margin-left: 10px"></i> </a>
                        @endcan
                        @can('dashboardworkcategorys_delete')
                        ...
                        @endcan
                        @can('dashboardworkcategorys_activate')
