...
<div class="panel panel-flat">
    <div class="panel-heading">
        <h5 class="panel-title"> {{__('contact.index')}} </h5>
    ...
    <table class="table datatable-button-init-basic" id="first">
        ...
        <tbody>
            ...
            <tr style="background:{{$item->seen=='No' ? '#e6a1a1':''}}">
                <td>{{$loop->index+1}}</td>
                <td>{{$item->name}}</td>
                <td>
                    {{$item->email}}
                ...
                {!!Form::open( ['route' => ['dashboard.contact.destroy',$item->id] ,'id'=>'delete-form'.$item->id, 'method' => 'Delete']) !!}
                {!!Form::close() !!}
                ...
                {!!Form::close() !!}
                <td>
                    @can('dashboardcontacts_edit')

                    <a href="{{route('dashboard.contact.show',[$item])}}" data-toggle="tooltip" data-original-title="show"> <i class="icon-eye text-inverse" style="margin-left: 10px"></i> </a>
                    @endcan
                    ...
                    @can('dashboardcontacts_activate')
