    ...
    public function activateanddeactive(Request $request)
    {
        ...
        if(class_exists($model) )
        {
          ...
          if( Schema::hasColumn($object->getTable(), 'status'))
          {
            ...
            alert()->success( __('Done').__('Status Changed'))->autoclose(5000);

          }else
          alert()->error( __('Done').__('Status Changed'))->autoclose(5000);


        }else{

            alert()->error( __('Done').'  '.__('Status Error'))->autoclose(5000);
    ...
    public function orderchangemodel(Request $request)
    {
        ...
        if(class_exists($model) )
        {
            ...
            if($modelid->order  > $tomodelid->order)
            {
             ...
             $res2 =   $modelid->save();
               alert()->success( __('Done').' '.__('Order Change'))->autoclose(5000);
               return redirect()->back();
